...
@Handler(supports={ObservationInMostRecentEncounter.class})
public class ObservationInMostRecentEncounterEvaluator implements RowPerPatientDataEvaluator{
	...
	public PatientDataResult evaluate(RowPerPatientData patientData, EvaluationContext context) {
		...
		ObservationInMostRecentEncounter pd = (ObservationInMostRecentEncounter)patientData;
        ...
        if(encounters != null)
        {
			...
			{
				boolean add = false;
				if(pd.getEncounterTypes().size()>0){
